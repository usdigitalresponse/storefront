...
export default function PromoBox(box: any, index: number, key: number) {
  return (
    <div style={styles.productBox} key={key}>
      <a href={`/box_details?id=${box.index}`}>
        <div style={{ ...styles.product, backgroundImage: `url(${box.box.image})` }}></div>
        <div style={styles.productDesc}>
          ...
          <span style={{ ...styles.out_of_order, display: out_of_stock(box) ? 'block' : 'none' }}>Out of Stock</span>
        </div>
      </a>
    </div>
  );
